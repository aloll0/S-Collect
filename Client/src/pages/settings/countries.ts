...
export const defaultCountry =
  countryOptions.find((country) => country.code === 'SA') ?? countryOptions[0];

export function getFlagUrl(code: string) {
  return 'https://flagcdn.com/w40/' + code.toLowerCase() + '.png';
}

export function getInitialPhone(phoneNumber: string) {
  const trimmedPhone = phoneNumber.trim();
  const country =
    countryOptions.find((option) => trimmedPhone.startsWith(option.dialCode)) ??
    defaultCountry;
  const localNumber = trimmedPhone.startsWith(country.dialCode)
    ? trimmedPhone.slice(country.dialCode.length).trim()
    : trimmedPhone;

  return { country, localNumber };
}

export function formatPhoneNumber(country: CountryOption, localNumber: string) {
  return `${country.dialCode} ${localNumber.trim()}`.trim();
}