...
const FAKE_ORDERS: Order[] = [
  {
    ...
    subtotal: 280, shippingFee: 35, discount: 0, tax: 47.25, grandTotal: 362.25,
    paymentStatus: "Paid",
    timeline: [
  ...
  },
  {
    ...
    subtotal: 1450, shippingFee: 0, discount: 100, tax: 202.5, grandTotal: 1552.5,
    paymentStatus: "Paid",
    timeline: [
  ...
  },
  {
    ...
    subtotal: 189, shippingFee: 35, discount: 0, tax: 33.6, grandTotal: 257.6,
    paymentStatus: "Paid",
    timeline: [
  ...
  },
  {
    ...
    subtotal: 245, shippingFee: 35, discount: 0, tax: 42, grandTotal: 322,
    paymentStatus: "Paid",
    timeline: [
  ...
  },
  {
    ...
    subtotal: 520, shippingFee: 35, discount: 50, tax: 75.75, grandTotal: 580.75,
    paymentStatus: "Paid",
    timeline: [
  ...
  },
  {
    ...
    subtotal: 380, shippingFee: 35, discount: 0, tax: 62.25, grandTotal: 477.25,
    paymentStatus: "Paid",
    timeline: [
  ...
  },
  {
    ...
    subtotal: 95, shippingFee: 35, discount: 0, tax: 19.5, grandTotal: 149.5,
    paymentStatus: "Paid",
    timeline: [
  ...
  },
  {
    ...
    subtotal: 1120, shippingFee: 35, discount: 0, tax: 173.25, grandTotal: 1328.25,
    paymentStatus: "Paid",
    timeline: [
  ...
  // Additional orders for testing pagination
  {
    ...
    subtotal: 650, shippingFee: 0, discount: 0, tax: 97.5, grandTotal: 747.5,
    paymentStatus: "Paid",
    timeline: [
  ...
  },
  {
    ...
    subtotal: 420, shippingFee: 35, discount: 0, tax: 68.25, grandTotal: 523.25,
    paymentStatus: "Paid",
    timeline: [
...
const FILTER_TABS: FilterTab[] = ["All Orders", ...ALL_STATUSES];
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center">
      <div className="bg-white rounded-2xl p-8 w-[420px] shadow-xl text-center">
        ...
        <h2 className="text-lg font-semibold text-gray-900 mb-1">Add Tracking Number</h2>
        <p className="text-sm text-gray-400 mb-5">Please enter the tracking number for this shipment to help the buyer track their order</p>
        <input
          type="text" placeholder="Enter tracking number" value={value}
        ...
        <div className="flex gap-3">
          <button onClick={() => onSave(value)} className="flex-1 bg-gray-900 text-white py-3 rounded-xl text-sm font-medium hover:bg-gray-700 transition-colors">
            save
          </button>
          <button onClick={onCancel} className="flex-1 border border-gray-200 text-gray-700 py-3 rounded-xl text-sm font-medium hover:bg-gray-50 transition-colors">
            Cancel
          </button>
        </div>
      </div>
...
const EmptyState = () => (
  <div className="flex flex-col items-center justify-center py-24 text-center">
    ...
    <p className="text-sm text-gray-400">They will appear here once you receive your first order</p>
  </div>
);
...
const OrderDetails = ({
  ...
  const handleUpdate = () => {
    ...
    // await fetch(`/api/orders/${order.id}`, { method: "PATCH", body: JSON.stringify({ status: selectedStatus, trackingNumber: tracking }) });
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 bg-[#f5f7fb]">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Order Details <span className="text-gray-500">#{order.id}</span></h1>
          <p className="text-xs text-gray-400 mt-0.5">
            Incoming Orders <ChevronRight size={12} className="inline" /> Order Details
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-[1fr_340px] gap-5 mt-5">
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            <h5 className="font-semibold text-gray-900 mb-4">Order Items</h5>
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100">
                  {["Product Name","Variant","SKU","Qty","Unit Price","Total Price"].map(h => (
                    <th key={h} className="text-left pb-2 text-xs text-gray-400 font-medium">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                ...
                <div key={i} className="flex gap-3">
                  <div className="flex flex-col items-center">
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${
                      t.done ? "bg-green-500 text-white" : "border-2 border-gray-200 bg-white text-gray-300"
                    }`}>
                      {t.done ? "✓" : ""}
                    </div>
                    {i < order.timeline.length - 1 && (
                      <div className={`w-px flex-1 my-1 ${t.done ? "bg-green-300" : "bg-gray-200"}`} style={{ minHeight: 24 }} />
                    )}
                  </div>
                  <div className="pb-5 flex-1 flex justify-between items-start">
                    <div>
                      <p className={`text-sm font-medium ${t.done ? "text-gray-900" : "text-gray-400"}`}>{t.step}</p>
                      {t.desc && <p className="text-xs text-gray-400 mt-0.5">{t.desc}</p>}
                    </div>
                    {t.date && <p className="text-xs text-gray-400 ml-4 whitespace-nowrap">{t.date}</p>}
                    {!t.date && !t.done && <p className="text-xs text-gray-300 ml-4">Pending</p>}
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5 text-sm">
            <div className="flex justify-between items-center mb-3">
              ...
              <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${STATUS_STYLES[order.status]}`}>{order.status}</span>
            </div>
            {[
              ["Order ID", `#${order.id}`],
              ...
              <div key={label} className="flex justify-between py-1.5 border-b border-gray-50 last:border-none">
                ...
                <span className={`font-medium ${label === "Payment Status" ? "text-green-600" : "text-gray-700"}`}>{val}</span>
              </div>
            ))}
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5 text-sm">
            ...
            {[["Name", order.customer.name], ["Email", order.customer.email], ["Phone", order.customer.phone]].map(([label, val]) => (
              <div key={label} className="flex justify-between py-1.5 border-b border-gray-50 last:border-none">
                <span className="text-gray-400">{label}</span>
                <span className="text-gray-700 font-medium">{val}</span>
              </div>
            ))}
            <p className="text-xs text-gray-300 mt-2">Phone number partially masked for privacy</p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5 text-sm">
            ...
            {[
              ["Subtotal", `SAR ${order.subtotal.toLocaleString()}.00`],
              ["Shipping Fee", `SAR ${order.shippingFee}.00`],
              ["Discount", `-SAR ${order.discount}.00`],
              [`Tax (15% VAT)`, `SAR ${order.tax.toFixed(2)}`],
            ].map(([label, val]) => (
          ...
          {/* Update Status */}
          <div className="bg-white rounded-2xl border border-gray-100 p-5 text-sm">
            <h5 className="font-semibold text-gray-900 mb-1">Update Order Status</h5>
            <p className="text-xs text-gray-400 mb-3">Status changes are irreversible and cannot be undone.</p>
            ...
            <label className="block text-xs text-gray-500 mb-1.5">Tracking Number (Optional)</label>
            <input
            ...
            {saved && (
              <div className="mt-3 flex items-center gap-2 bg-green-50 border border-green-200 text-green-700 rounded-xl px-4 py-2.5 text-sm">
                <span className="text-green-500">✓</span> Order updated successfully.
              </div>
            )}
          </div>
        </div>
...
const IncomingOrders = () => {
  ...
  const [trackingModal, setTrackingModal] = useState<string | null>(null); // orderId
  const [currentPage, setCurrentPage] = useState(1);

  const filtered = useMemo(() => {
    const list = orders.filter(
      (o) => activeTab === "All Orders" || o.status === activeTab
    );
  ...
  const applyStatusChange = (orderId: string, newStatus: OrderStatus, tracking: string) => {
    // TODO: API call
    // await fetch(`/api/orders/${orderId}/status`, { method: "PATCH", body: JSON.stringify({ status: newStatus, trackingNumber: tracking }) });
    setOrders((prev) =>
      prev.map((o) =>
        o.id === orderId ? { ...o, status: newStatus, trackingNumber: tracking || o.trackingNumber } : o
      )
    );
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 bg-[#f5f7fb]">
      ...
      <h1 className="text-3xl font-bold text-gray-900 mb-6">Incoming Orders</h1>
        ...
        {paginatedData.length === 0 ? (
          ...
          <>
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100">
                  {["Order ID","Order Date","Customer Name","Total Amount","Order Status","Actions"].map(h => (
                    <th key={h} className="text-left py-3 px-2 text-xs text-gray-400 font-medium">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {paginatedData.map((order) => (
                  <tr key={order.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-4 px-2 font-medium text-gray-900">{order.amount.toLocaleString()} SAR</td>
