...
const InventoryCard = ({ cardData }: { cardData: CardData }) => {
  ...
  return <div className='flex  items-center gap-3 shadow'>
    ...
    <div className="flex flex-col gap-2.5 flex-1 ">
      ...
      <p className='text-sm text-gray-400' >{t('inventoryItem.sku')} : {cardData.sku}</p>
    </div>
    <div className="flex flex-col  items-end pe-1.5">
      <span className="py-1 px-2 rounded-full inline-block mb-3" style={{ background: cardData.theme.background, color: cardData.theme.text }} >  {getStatusLabel(cardData.status)}</span>
      <span className="text-gray-400" > {t('inventoryItem.stock')} : {cardData.stockCount}</span>
    </div>
  </div>;
};
